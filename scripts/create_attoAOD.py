...
if __name__ == "__main__":
    ...
    parser.add_option("-m", "--maxFiles", dest="MaxFiles", type=int, default=-1,
    ...
    def read_file_list(FileList, MaxFiles, Batch):
        ...
        # startfile = 0
        # endfile = nInputFiles
        startfile = int(Batch)*int(MaxFiles) #changed for hex running over crab datasets
    ...
    if OutputFile == "":
        ...
        if "_" in OutputFile and OutputFile.split("_")[-1].isdigit():
            OutputFile = "_".join(OutputFile.split("_")[:-1])  # Remove "_filelistIndex", if it exists
        OutputFile = str(Batch)+"_"+ OutputFile + ".root"
        print(OutputFile)
